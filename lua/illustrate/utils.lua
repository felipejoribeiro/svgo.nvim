...
function M.insert_include_code(filename, caption)
  local insert_code = Config.options.text_templates.svg.md
    :gsub('$FILE_PATH', filename)
    :gsub('$CAPTION', caption .. '.svg')
...
function M.copy_from_template(template_path, destination_path)
